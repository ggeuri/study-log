...
public class ShopWebConfig extends WebMvcConfigurerAdapter {
	...
	public void addInterceptors(InterceptorRegistry registry) {
		registry.addInterceptor(new LoginCheckInterceptor()).addPathPatterns("/**").excludePathPatterns("/",
				"/member/loginform", "/member/logout", "/product/list", "/product/detail", "/oauth2/authorize/naver",
				"/oauth2/authorize/google", "/oauth2/authorize/kakao", "/login/callback/naver", "/login/callback/kakao",
				"/login/callback/google","/static/**");
	}
	...
	// Oauth로그인시 사용되는 환경변수는 객체로 담아 관리하면 유지하기 좋음. 여러프로바이더 연동하니 보관해놓자 OAuthClient객체를
	// 여러개 메모리 보관
	@Bean
