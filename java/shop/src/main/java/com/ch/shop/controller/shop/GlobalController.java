...
/*
	...
	용도 : 쇼핑몰의 모든 컨트롤러에서 현재 TopCategory의 목록을 매번 가져오는 코드가 중복되고 있으므로, 이를 해결하기 위함  
 	해결 : @ControllerAdvice 명시 
 			ControllerAdivce 안에 작성된 메서드는 다른 컨트롤러의 실행에 앞서서 실행되어 지는데, 주의할점은 이안에 정의한 모든 메서드
 			가 동작하는 먼저 것이 아니라, 아래의 3가지 어노테이션에 대해서만 효과가 발생 
 			1) @InitBinder : 컨트롤러로 전달되는 파라미터에 대해 개발자가 커스텀하고 싶을때 사용 
 			2) @ModelAttribute : Model 객체에 model.setAttribute("topList", topList); 의 효과를 낼수 있는 어노테이션 
 			3) @ExceptionHandler 
*/
