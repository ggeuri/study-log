...
public class Service {
    ...
    public void addTransfer(){
        ...
        //날짜타입으로만 입력할 수 있게 고쳐보자 format.setLenient(false);  엄격모드..
        ...
        try{dateResult = formatter.parse(date);
            date = formatter.format(dateResult);}
        catch(ParseException e){
            IoManager.print("잘못된 날짜 형식입니다.(ex)2025-01-01");    
            return;        
        }

        //수입 혹은 지출만 쓸 수 있게 고쳐보자 
 
        String inoutType = null;
        String inoutTypeResult = IoManager
                                .input("수입, 지출 여부를 입력하세요(수입 혹은 INCOME/ 지출 혹은 EXPENSE) > ")
                                .trim()
                                .toUpperCase();

        if(inoutTypeResult.equals("수입")||inoutTypeResult.equals("INCOME")){
            inoutType = "수입"; 
        } else if (inoutTypeResult.equals("지출")||inoutTypeResult.equals("EXPENSE")){
            inoutType = "지출"; 
        } else {
            IoManager.print("잘못된 입력입니다.(수입 혹은 INCOME/ 지출 혹은 EXPENSE)");
            return;}
        ...
        int amount = 0 ; 
        try{ 
            amount = Integer.parseInt(IoManager.input("금액을 입력하세요 > "));
        }
        catch(NumberFormatException e) {
            IoManager.print("금액을 잘못 입력하셨습니다.");
            return; 
        }
    ...
    public void deleteTransfer(){
         ...
         if(deleteIndex == 0 || deleteIndex > list.size()){
            IoManager.print("잘못된 번호 입력입니다. 다시 선택해주세요");
            return;
         } else{
