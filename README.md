...
| 2026-01-20 | src/main/java/com/ch/noticeapp/config/WebConfig.java, src/main/java/com/ch/noticeapp/notice/controller/MybatisNoticeController.java | React(7777)에서 API(9999) 호출 시 브라우저 CORS 차단으로 등록/조회가 막혀 기능 검증이 불가했고, REST 경로/메서드가 일관되지 않으면 클라이언트 연동이 쉽게 깨질 위험이 있었음 | WebConfig에 /api/** 범위로 CORS를 제한 적용하고 allowedOrigins(http://localhost:7777) + allowedMethods + allowedHeaders로 로컬 프론트만 통과시키도록 구성함. 컨트롤러는 /api/notices 기준으로 REST 규칙(명사형 리소스, HTTP 메서드 의미)을 맞추고, POST는 ResponseEntity.created(Location)로 생성 결과를 명확히 반환하도록 정리해 연동 안정성을 확보함. |
| 2026-01-21 | web/index.html, noticeapp/src/main/java/com/ch/noticeapp/notice/* | React CRUD에서 등록/수정/삭제 후 화면이 즉시 갱신되지 않거나 선택 상세가 꼬일 위험이 있음 | App을 SoT로 두고 fetch 이후 setState를 불변성(map/filter/spread)으로 갱신하여 즉시 리렌더되게 정리, DetailForm은 props.notice를 로컬 state로 복사(useEffect)해 편집 중 부모 상태 오염을 방지함 |
| 2026-01-22 | Projects/MyProjects/study-log (JPA+WebSocket+Nginx) | JPA 트랜잭션/더티체킹과 WebSocket(JSON 직렬화)·Nginx 프록시 흐름이 섞이면 “어디서 처리되는지”가 헷갈려 디버깅 시간이 커짐 | Controller는 바인딩/응답, Service는 트랜잭션 내 영속성 변경감지로 역할을 분리하고, WebSocket은 ObjectMapper로 JSON↔DTO 변환 후 서버시간을 부여해 메시지 스키마를 고정했다. Nginx는 정적 서빙과 /api 프록시를 분리해 호출 경로를 단순화했다. |
| 2026-01-23 | server/src/main/java/com/ch/stompserver/member (SecurityConfig, AuthController, CustomDetailsService) | fetch 로그인 기반으로 세션 인증을 붙이지 않으면 /me 같은 보호 API가 익명 처리되어 채팅 사용자 식별이 깨지고 라우팅/권한 통제가 불가능해지는 리스크 | AuthenticationManager로 인증을 위임하고 성공 시 SecurityContext를 세션에 저장해 재요청에서 Authentication을 복원하도록 구성. CORS에 allowCredentials(true)와 허용 Origin을 명시해 브라우저가 쿠키를 전송하게 했고, /api/auth/**만 permitAll로 열어 인증 경로와 보호 자원을 분리해 보안 사고 가능성을 낮춤. |
| 2026-01-26 | server/src/main/java/com/ch/stompserver/chat (ChatController, StompEventListener) + client/main.html | STOMP 채팅에서 방(roomId) 변경/재입장 시 구독이 누적되면 동일 메시지가 중복 수신되어 UI/운영 로그가 오염될 위험 | 클라이언트에서 새 room subscribe 전에 기존 subscription을 unsubscribe하고 ref를 초기화하여 “구독 1개 유지”로 정리. 서버는 SimpMessagingTemplate.convertAndSend로 topic을 단일 소스로 브로드캐스트하고, EventListener로 connect/subscribe/unsubscribe를 로그화해 운영 관측 가능성을 확보함. |
| 2026-01-27 | securityapp/…/SecurityConfig.java + CustomDetailsService.java + front/viteapp/src/api/auth.js | SPA 로그인 요청이 시큐리티 필터 체인에서 사용자 조회 미구현(null 반환)으로 실패하고 302 리다이렉트로 이어져 fetch/CORS 문제처럼 보이는 리스크 | BeforeParameterFilter로 파라미터 유입을 확인한 뒤 UserDetailsService 계약 위반(null 반환)을 원인으로 특정. MemberRepository+CustomUserDetails로 사용자 조회/반환을 구현하고, 성공 시 JsonSuccessHandler로 JSON 응답을 내려 SPA에서 결과를 안정적으로 처리하도록 정리함. |
⸻
